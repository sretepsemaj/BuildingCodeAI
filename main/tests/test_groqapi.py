...
class TestGroqAPI(TestCase):
    def setUp(self):
        ...
        self.test_image_path = os.path.join(settings.BASE_DIR, 'main/tests/test_files/test_image.jpg')
    ...
    def test_json_mode(self, mock_groq_class):
        ...
        mock_completion = MagicMock()
        mock_completion.choices = [
            MagicMock(message=MagicMock(content='{"objects": ["test object"], "description": "test description"}'))
        ]
        mock_groq_class.return_value.chat.completions.create.return_value = mock_completion
